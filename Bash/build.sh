...
sudo mkdir gold
cd gold/
sudo git clone https://github.com/sagarkakkalasworld/Day4.git
cd Day4/Gold-site-Ecommerce
sudo docker build -t react-microk8s -f golddockerfile .
sudo docker tag react-nginx:latest sagarkakkalasworld/react-microk8s:latest ##make sure you did docker login
